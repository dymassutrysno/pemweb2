  ...
  <form action="paramedik_update.php" method="POST">
    ...
    <div class="form-group row">
      ...
      <div class="col-8">
        ...
        <div class="custom-control custom-radio custom-control-inline">
          <input name="kategori" id="kategori_" type="radio" class="custom-control-input" value="S3" required="required" <?= $pasien["kategori"] == 'S3' ? 'checked' : '' ?>>
    ...
    <div class="form-group row">
      <label for="unit_id_kerja" class="col-4 col-form-label">Kelurahan</label>
      <div class="col-8">
        <select id="unit_id_kerja" name="unit_id_kerja" class="custom-select" required="required">
          <option value="">-- Pilih Unit Kerja --</option>
          <?php foreach ($unit_id_kerja as $kel) : ?>
            <option <?= $pasien["unit_id_kerja" ] == $kel['id'] ? 'selected' : '' ?> value="<?= $kel['id'] ?>"><?= $kel['nama'] ?></option>
          <?php endforeach; ?>
        </select>
      </div>
    </div>
